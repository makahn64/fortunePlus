...
function route(name, model) {
  ...
  router.get(collectionRoute, function (req, res) {
    ...
    //MAK Cutting in direct nedb search
    var neSearch = {};
    if (req.query.$find!==undefined){
        neSearch = JSON.parse(req.query.$find);
    }

    // get resources by IDs

    // MAK added neSearch
    // The 5000 is a big limit, though the original code didn't use it at all limiting to 1000!
    adapter.findMany(model, ids, 5000, neSearch)
